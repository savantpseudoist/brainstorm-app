...
const ACTION_TEXT: Record<AIAction, string> = {
  brainstorm:
    'Brainstorm a wide range of fresh, distinct ideas on this topic. Give me a numbered list, favouring variety and unexpected angles over polish.',
  expand:
    'Take the notes below and expand on the most promising directions. For each, add concrete detail, next steps, and one risk to watch.',
  critique:
    'Play devil\'s advocate on the notes below. Point out weak assumptions, gaps, and failure modes, then suggest how to address each.',
  ask: 'Here are my brainstorming notes so far.'
}
